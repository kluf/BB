...
App.Views.WeekRow = Backbone.View.extend({
    ...
    render: function() {
        ...
        month.weekDays(this.week).forEach(function(date) {
...
App.Views.DayTable = Backbone.View.extend({
    ...
    render: function() {
        ...
        for (var i = 0; i < 24; i += 1) {
            var time = moment(i, "H").utc().format('h:mm A');
            this.hours[time] = new App.Views.Hour({
                time: time
            });
...
App.Views.CreateEvent = Backbone.View.extend({
    ...
    createEvent: function(evt) {
        ...
        var model = new App.Models.Event({
            ...
            start: this.$('#eventStartTime').val(),
            end: this.$('#eventEndTime').val(),
        });
        if (model.isValid()) {
